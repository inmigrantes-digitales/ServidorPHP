...
| Método | URL            | Auth | Descripción                                   |
|--------|----------------|------|-----------------------------------------------|
| POST   | `/api/proxy`   | No   | Proxy para consultar otros servidores HTTP(S) |

#### POST `/api/proxy`

Permite hacer consultas a otros servidores HTTP(S) agregando automáticamente credenciales y preservando el token JWT del usuario actual.
...
**Lo que hace:**
- Agrega **HTTP Basic Auth** si `PROXY_USER` y `PROXY_PASSWORD` están configuradas en `.env`.
- Preserva el token JWT del usuario en header `X-User-Token` (para identificación).
- Propaga headers autorizados (Content-Type, Accept, etc).
- Valida URLs y evita SSRF básico.
- Maneja JSON y texto plano.

**Configuración en `.env`:**
```env
PROXY_USER=tu_usuario
PROXY_PASSWORD=tu_clave
```

**Uso desde frontend:**
```javascript
const response = await fetch('https://tu-dominio/api/proxy', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  ...
  body: JSON.stringify({
    url: 'https://otro-servicio.com/api/datos',
    method: 'GET'
  })
});
...
**Nota:** El token JWT del usuario se propaga automáticamente, así que el servidor destino puede identificar al usuario si revisa el header `X-User-Token`.
